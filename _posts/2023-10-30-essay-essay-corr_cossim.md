...
즉, 변수가 2개인 경우 공분산은 $$\bar{X}$$ 와 $$\bar{Y}$$ 로 좌표평면을 나눴을 때 데이터 포인트가 **1, 3 사분면**에 많을 땐 양수, **2, 4 사분면**에 많을 땐 음수가 나오게 된다. 따라서 두 변수가 양의 선형 관계를 나타낼 때 공분산은 양수, 음의 선형 관계를 나타낼 때 공분산은 음수이다.
