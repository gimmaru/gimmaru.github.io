...
다양한 주식 관련 앱을 찾아보니, 앱 별로 제공하는 기능 간 차별화되는 부분은 크게 없었던 것 같다. 기존에 확인할 수 있는 기능들은 특허로 인정받을 정도로 차별성이 있진 않을 것 같고, 특허 침해 문제와는 큰 연관이 없을 것 같다는 느낌을 받았다. 이와 별개로 알파스퀘어와 위불 앱의 PC UI가 매우 비슷했는데, 디자인 특허 관련 시비가 생길 가능성도 있을 것 같다. 세부적으로 비슷한 기능이 아니라면 기능보다는 디자인을 주의하는 것이 중요해 보인다. 만약 내 앱만의 독창적인 UI를 생각해 낸다면 디자인 특허 출원 방법은 꼭 더 알아봐야겠다.
