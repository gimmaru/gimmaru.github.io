...
(<button onClick="window.location.reload()">수식이 깨진다면 클릭</button>)

모델에 대한 입력: $$X$$

모델의 출력: $$Y$$

$$P(Y|X)$$ 는 입력이 주어졌을 때 출력의 조건부 확률을 나타낸다. 
$$P(X)$$ 는 입력의 확률 밀도 함수를 나타내며 
$$P(Y)$$ 는 출력의 확률 밀도 함수를 나타낸다.
...
관심 있는 분포를 모니터링할 수도 있다. 일반적인 관심 분포는 $$P(X)$$,
 $$P(Y)$$, 
 $$P(X|Y)$$, 
 $$P(Y|X)$$ 이다.
